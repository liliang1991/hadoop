...
public class FileBlockManager {
   public static final Log LOG = LogFactory.getLog(FileBlockManager.class);
    ...
    Set<Block> postponedMisreplicatedBlocks = Sets.newHashSet();
   ...
   FileHeartbeatManager heartbeatManager;
     ...
     * what happened with it.
     */
    public MisReplicationResult processMisReplicatedBlock(BlockInfo block) {
   ...
   /*     if (bc == null) {
    ...
    private LocatedBlock createLocatedBlock(final BlockInfo[] blocks,
                                            final long endPos, final BlockTokenSecretManager.AccessMode mode) throws IOException {
        int curBlk = 0;
        long curPos = 0;
        int nrBlocks = (blocks[0].getNumBytes() == 0) ? 0 : blocks.length;
        for (curBlk = 0; curBlk < nrBlocks; curBlk++) {
  ...
  public   enum MisReplicationResult {
    ...
    void addStoredBlockUnderConstruction(
            ...
            DatanodeDescriptor node,
            HdfsServerConstants.ReplicaState reportedState)
            throws IOException {
...
assert storedBlock.findDatanode(dn) < 0 : "Block " + block
            ...
            switch (rdbi.getStatus()) {
                ...
                case RECEIVING_BLOCK:
                    ...
                    processAndHandleReportedBlock(node, rdbi.getBlock(),
                            HdfsServerConstants.ReplicaState.RBW, null);
                    break;
                default:
                    String msg =
    ...
    private void processAndHandleReportedBlock(DatanodeDescriptor node, Block block,
                                               HdfsServerConstants.ReplicaState reportedState, DatanodeDescriptor delHintNode)
            throws IOException {
